...
if ( ! class_exists( 'Directory_Post_Types' ) ) {
    ...
    class Directory_Post_Types {
        ...
        public function __construct() {
            ...
            // NEW: Flush rewrite rules on theme switch to prevent 404 errors
        ...
        public function register_post_types() {
            ...
            $listing_args = [
                ...
                'hierarchical'       => false,
                'menu_position'      => 5,
                // FIX: Added 'editor' and 'thumbnail' support for content and featured image
                'supports'           => [ 'title', 'editor', 'author', 'thumbnail' ], 
                'menu_icon'          => 'dashicons-list-view',
            ];
            register_post_type( 'wpd_listing', $listing_args );
            
            // FIX: Flush rewrite rules to ensure single post type URLs work correctly
            flush_rewrite_rules();
            ...
            <div id="wpd-taxonomy-builder-wrapper">
                <div id="wpd-taxonomies-container">
                    ...
                    <div class="wpd-field-row">
                        ...
                        <a href="#" class="button wpd-remove-field" style="margin: 10px;"><?php _e( 'حذف', 'wp-directory' ); ?></a>
                    </div>
        ...
        public function render_field_builder_metabox( $post ) {
            ...
            // START OF NEW FEATURES: Import/Export UI
            ?>
        ...
        private function render_field_builder_row($index, $field_data = [], $all_fields = []) {
            ...
            <div class="wpd-field-row" data-index="<?php echo esc_attr($index); ?>" data-field-key="<?php echo esc_attr($key); ?>">
                <div class="wpd-field-header">
                    ...
                    <!-- START OF NEW FEATURES: Copy Field Icon -->
        ...
        public function save_listing_type_meta_data( $post_id ) {
            ...
            // START OF NEW FEATURES: Handle field builder import
            if (isset($_FILES['wpd_import_fields_file']) && !empty($_FILES['wpd_import_fields_file']['tmp_name'])) {
                ...
                if ($file['type'] === 'application/json') {
                    ...
                    if (is_array($fields_data)) {
                        ...
                        // We return here to avoid being overwritten by the regular save process below
                        return;
                    }
                }
            }
        ...
        private function process_and_save_fields($post_id, $field_definitions, $posted_data, $meta_prefix = '_wpd_') {
            foreach ($field_definitions as $field_def) {
                ...
                if (!$value_exists) {
                    if (in_array($field_def['type'], ['checkbox', 'multiselect', 'repeater', 'social_networks', 'simple_list', 'gallery', 'product'])) {
                ...
                <script type="text/javascript">
                    jQuery(document).ready(function($) {
                        ...
                        function getNewIndex() {
                            ...
                            return container.children().length ? (Math.max.apply(null, container.children().map(function() { return $(this).data('index'); }).get()) + 1) : 0;
                        }
                ...
                <div class="wpd-admin-field-wrapper <?php echo esc_attr($width_class); ?>" <?php echo $wrapper_attributes; ?>>
                    ...
                    <div class="wpd-field-input-wrapper">
                        ...
                        switch($field['type']) {
                            ...
                            case 'date':
                                echo '<input type="text" id="' . esc_attr($field_id) . '" name="' . esc_attr($field_name) . '" value="' . esc_attr($value) . '" class="regular-text wpd-date-picker" autocomplete="off">';
                                break;
