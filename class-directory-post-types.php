    ...
    class Directory_Post_Types {
        ...
        public function register_post_types() {
            ...
            $listing_args = [
                ...
                'hierarchical'       => false,
                'menu_position'      => 5,
                // FIX: Support for `editor` and `thumbnail` removed as requested
        ...
        public function remove_default_meta_boxes() {
            // FIX: Remove editor meta box from the listing post type and thumbnail
        ...
        private function render_field_builder_row($index, $field_data = [], $all_fields = []) {
            ...
            $show_in_frontend = $field_data['show_in_frontend'] ?? 1; // NEW: Default is 1 (visible)
            $width_class = $field_data['width_class'] ?? 'full';
            $help_text = $field_data['help_text'] ?? ''; // NEW
            $placeholder = $field_data['placeholder'] ?? ''; // NEW
            $default_value = $field_data['default_value'] ?? ''; // NEW
            ...
            <div class="wpd-field-row" data-index="<?php echo esc_attr($index); ?>" data-field-key="<?php echo esc_attr($key); ?>">
                ...
                <div class="wpd-field-details" style="display:none;">
                    ...
                    <!-- NEW: Fix for file, image, and gallery fields to always render but be hidden -->
        ...
        private function sanitize_field_builder_data($fields) {
            ...
            foreach ($fields as $field) {
                ...
                $sanitized_field = [
                    ...
                    'show_in_frontend' => isset($field['show_in_frontend']) ? 1 : 0, // NEW
                    'width_class' => isset($field['width_class']) ? sanitize_key($field['width_class']) : 'full',
                    'help_text' => isset($field['help_text']) ? sanitize_text_field($field['help_text']) : '', // NEW
                    'placeholder' => isset($field['placeholder']) ? sanitize_text_field($field['placeholder']) : '', // NEW
                    'default_value' => isset($field['default_value']) ? sanitize_text_field($field['default_value']) : '', // NEW
                ];
        ...
        private function process_and_save_fields($post_id, $field_definitions, $posted_data, $meta_prefix = '_wpd_') {
            // New logic to handle setting the featured image
            ...
            foreach ($field_definitions as $field_def) {
                ...
                if (in_array($field_def['type'], ['repeater', 'social_networks', 'simple_list'])) {
                    ...
                    // If this field is set to be the featured image, set it
            ...
            // Set the featured image after saving all other post meta
                ...
                <script type="text/javascript">
                    jQuery(document).ready(function($) {
                        ...
                        $('#wpd-add-field').on('click', function(e) {
                            ...
                            // New fields are type 'text', so hide file settings initially
                            field_html.find('.wpd-file-settings').hide();
                            updateConditionalFieldDropdowns();
                        });
                        
                        // NEW: Handler for adding new taxonomy row
                    ...
                    jQuery(document).ready(function($) {
                        ...
                        function initializeWpdComponents(container) {
                            container.find('.wpd-upload-gallery-button:not([data-initialized])').each(function() {
                             ...
                             // NEW: File upload button handler
                            container.find('.wpd-upload-file-button:not([data-initialized])').each(function() {
                             ...
                             // NEW: File remove button handler
                        ...
                        // Initial load of components for the current listing type
        ...
        private function render_admin_fields_recursive($fields, $post_id, $row_data = [], $name_prefix = 'wpd_custom', $meta_prefix = '_wpd_') {
            foreach ($fields as $field) {
                ...
                // NEW: Use default value if a value is not yet set
                if (empty($value)) {
                    $default_value = $field['default_value'] ?? '';
                    // Check if it's a shortcode and process it
                    if (strpos($default_value, '[') !== false && strpos($default_value, ']') !== false) {
                        $value = do_shortcode($default_value);
                    }
                    // Check if it's a meta key and get the value
                    if (strpos($default_value, '{') !== false && strpos($default_value, '}') !== false) {
        ...
        // START OF NEW FEATURES: New functions for admin features
