    ...
    class Directory_Post_Types {
        ...
        public function register_post_types() {
            ...
            $type_args = [
                ...
                'show_in_menu'  => 'wpd-main-menu',
                'capability_type' => 'post',
                'supports'      => [ 'title', 'slug' ], // *** FIX: 'editor' removed to hide the content editor.
            ];
        ...
        public function enqueue_admin_scripts($hook) {
            ...
            if ( ($pagenow == 'post-new.php' || $pagenow == 'post.php') && in_array($post_type, ['wpd_listing', 'wpd_listing_type']) ) {
                ...
                if(!empty($listing_types)) {
                    foreach($listing_types as $type_post) {
                        ...
                        if(!empty($taxonomies) && is_array($taxonomies)) {
                            $all_dynamic_tax_slugs = array_merge($all_dynamic_tax_slugs, wp_list_pluck($taxonomies, 'slug'));
                        }
                    }
                }
                ...
                wp_localize_script('jquery', 'wpd_admin_params', [
                    ...
                    'all_dynamic_taxonomies' => array_unique($all_dynamic_tax_slugs)
                ]);
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    if(response.success) {
                                        ...
                                        updateTaxonomyMetaboxes(response.data.taxonomies);
                                    } else {
                                        fields_container.html('<p style="color:red;">' + (response.data.message || 'خطای ناشناخته.') + '</p>');
        ...
        public function ajax_load_admin_fields_and_taxonomies() {
            ...
            if ( ! current_user_can('edit_posts') ) {
                wp_send_json_error(['message' => __('شما دسترسی لازم را ندارید.', 'wp-directory')]);
            }
            ...
            if(empty($type_id)) {
                wp_send_json_success([
                    'fields' => '<p>' . __('لطفا یک نوع آگهی انتخاب کنید.', 'wp-directory') . '</p>', 
                    'taxonomies' => []
                ]);
            }

            $fields_html = $this->get_admin_fields_html($type_id, $post_id);
            $taxonomies_meta = get_post_meta($type_id, '_defined_taxonomies', true);
            $tax_slugs = !empty($taxonomies_meta) && is_array($taxonomies_meta) ? wp_list_pluck($taxonomies_meta, 'slug') : [];
