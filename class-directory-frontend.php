    ...
    class Directory_Frontend {
        ...
        private function _render_frontend_field_recursive($fields, $post_id, $row_data = [], $name_prefix = 'wpd_custom') {
            foreach ($fields as $field) {
                // NEW: Check if the field should be displayed on the frontend
                ...
                // NEW: Handle default value for frontend form
                if (empty($value) && !empty($field['default_value'])) {
                    ...
                    if (strpos($default_value, '[') !== false && strpos($default_value, ']') !== false) {
                        ...
                        // We can't fetch meta keys for a non-existent post, but we can for the current user
