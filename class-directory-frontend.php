...
if ( ! class_exists( 'Directory_Frontend' ) ) {
    ...
    class Directory_Frontend {
        ...
        public function __construct() {
            ...
            // NEW: Filter to force single listing template.
            add_filter('single_template', [$this, 'set_listing_single_template']);
            // NEW: Filter to force archive listing template.
        ...
        public function enqueue_scripts() {
            ...
            wp_enqueue_script( 'wpd-main-script', WPD_PLUGIN_URL . 'assets/js/main.js', [ 'jquery' ], WPD_PLUGIN_VERSION, true );
            ...
            // NEW: Enqueue WP Datepicker and Leaflet for frontend forms
        ...
        // NEW: Force single listing template
        public function set_listing_single_template($template) {
            global $post;
            if ($post->post_type == 'wpd_listing') {
                $plugin_template = WPD_PLUGIN_PATH . 'templates/single-wpd_listing.php';
        ...
        // NEW: Force archive listing template
        public function set_listing_archive_template($template) {
            ...
            if (is_post_type_archive('wpd_listing') || is_tax('wpd_listing_category') || is_tax('wpd_listing_location')) {
                $plugin_template = WPD_PLUGIN_PATH . 'templates/archive-wpd_listing.php';
                 if (file_exists($plugin_template)) {
                    return $plugin_template;
                }
            }
        ...
        public function register_shortcodes() {
            ...
            // NEW: Shortcode for generic archive page, with optional 'type' parameter for specific listing types
            add_shortcode( 'wpd_listing_archive', [ $this, 'render_archive_page' ] );

            // NEW: Dynamic shortcodes for each listing type
        ...
        /**
         * Renders the submit listing form.
         *
        ...
        public function render_submit_form($atts) {
            ...
            // FIX: Prioritize type from URL param if exists
            $listing_type_id = isset($_GET['listing_type_id']) ? intval($_GET['listing_type_id']) : (!empty($atts['type']) ? intval($atts['type']) : 0);
        ...
        /**
         * Renders the listing archive page.
         *
         * @param array $atts Shortcode attributes.
         * @return string The archive page HTML.
         */
        ...
        /**
         * Get listings HTML based on arguments.
         *
        ...
        /**
         * Renders a single listing item HTML for archive pages.
         *
         * @param int $post_id The listing post ID.
         * @return string The listing item HTML.
         */
        private function _render_listing_item($post_id) {
            ob_start();
            $post = get_post($post_id);
            if ($post) {
                // NEW: Use template file from within plugin directory
        ...
        /**
         * Renders the main submit form or edit form.
         *
        ...
        private function _display_main_submit_form( $listing_id = 0, $listing_type_id = 0, $package_id = 0 ) {
            // FIX: Correctly get listing type from URL parameter or post meta
            ...
            if (empty($listing_type_id)) {
                ...
                if (count($listing_types) > 1) {
                    // FIX: Render selection form directly without redirecting
            ...
            // NEW: Get base cost and currency for payment summary
        ...
        /**
         * Renders the listings tab in the user dashboard.
         */
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="<?php echo add_query_arg(['wpd_action' => 'delete_listing', 'listing_id' => get_the_ID(), '_wpnonce' => wp_create_nonce('wpd_delete_listing_nonce')]); ?>" onclick="return confirm('<?php _e('آیا مطمئن هستید؟', 'wp-directory'); ?>');"><?php _e('حذف', 'wp-directory'); ?></a>
                        </td>
                    </tr>
        ...
        /**
         * Renders the transactions tab in the user dashboard.
         */
        ...
        /**
         * Renders the profile tab in the user dashboard.
         */
        ...
        /**
         * Renders the filter form HTML for archive page.
         *
         * @param int $listing_type_id Optional. The ID of the listing type.
         * @return string The filter form HTML.
         */
        private function _get_filter_form_html($listing_type_id = 0) {
            ob_start();
            // NEW: Use template file for filter form
        ...
        public function handle_form_submission() {
            ...
            // NEW: Handle taxonomies from form submission
            $taxonomies = get_post_meta($listing_type_id, '_defined_taxonomies', true);
            $global_taxonomies = ['wpd_listing_category', 'wpd_listing_location'];
            $all_taxonomies = array_merge($global_taxonomies, wp_list_pluck($taxonomies ?? [], 'slug'));
        ...
        /**
         * Handle listing actions (delete, renew, etc.) from user dashboard.
         */
        ...
        /**
         * AJAX handler to load custom fields for the submit form.
         */
        ...
        /**
         * Renders the custom fields HTML for the submit form.
         *
         * @param int $listing_type_id
         * @param int $listing_id
         * @return string
        ...
        /**
         * Recursively renders a single frontend field based on its type.
         *
         * @param array $fields Field definitions.
         * @param int $post_id Post ID for retrieving values.
         * @param array $row_data Data for repeater rows.
        ...
        /**
         * AJAX handler to filter listings.
         */
        ...
        /**
         * AJAX handler to load a filter form for a specific listing type.
         */
        ...
        /**
         * Renders the HTML content for the dynamic filter form.
         * This function is now public to be called by the shortcode.
         */
        ...
        private function _locate_template($template_name) {
            $path = WPD_PLUGIN_PATH . 'templates/' . $template_name;
