...
if ( ! class_exists( 'Directory_Frontend' ) ) {
    ...
    class Directory_Frontend {
        ...
        public function enqueue_scripts() {
            ...
            $main_font = Directory_Main::get_option('appearance', [])['global_main_font'] ?? 'vazir';
            ...
            $submit_page_id = Directory_Main::get_option('general', [])['submit_page'] ?? 0;
            $dashboard_page_id = Directory_Main::get_option('general', [])['dashboard_page'] ?? 0;

            if ( is_page( $submit_page_id ) || is_page( $dashboard_page_id ) || is_singular('wpd_listing') ) {
        ...
        public function set_listing_archive_template($template) {
            if (is_post_type_archive('wpd_listing') || is_tax(get_object_taxonomies('wpd_listing'))) {
        ...
        public function get_listings_html($args) {
            ...
            <div class="wpd-listings-container">
                ...
                <?php endwhile; wp_reset_postdata(); else: ?>
                    <div class="wpd-alert wpd-alert-warning"><?php _e('هیچ آگهی با این مشخصات یافت نشد.', 'wp-directory'); ?></div>
                <?php endif; ?>
            </div>
                ...
                <form method="post" enctype="multipart/form-data">
                    ...
                    foreach($all_taxonomies as $tax_slug){
                        ...
                        if ($is_hierarchical) {
                            ...
                            echo '<select id="' . esc_attr($tax_slug) . '" name="' . esc_attr($tax_slug) . '[]" multiple>';
                            foreach($terms as $term){
                                $selected = in_array($term->term_id, $selected_terms) ? 'selected' : '';
                                echo '<option value="' . esc_attr($term->slug) . '" ' . $selected . '>' . esc_html($term->name) . '</option>';
                            }
        ...
        public function handle_form_submission() {
            ...
            foreach($all_taxonomies as $tax_slug){
                if(isset($form_data[$tax_slug])){
                    $terms_to_set = is_array($form_data[$tax_slug]) ? array_map('sanitize_title', $form_data[$tax_slug]) : sanitize_title($form_data[$tax_slug]);
                    wp_set_object_terms($listing_id, $terms_to_set, $tax_slug, false);
                } else {
        ...
        private function _render_frontend_field_recursive($fields, $post_id, $row_data = [], $name_prefix = 'wpd_custom') {
            foreach ($fields as $field) {
                if (!isset($field['show_in_frontend']) || $field['show_in_frontend'] == 0) {
                    continue;
                }
                ...
                if (empty($value) && !empty($field['default_value'])) {
                    ...
                    if (strpos($default_value, '[') !== false && strpos($default_value, ']') !== false) {
                        ...
                        if (get_current_user_id()) {
                            ...
                            $value = get_user_meta(get_current_user_id(), $meta_key_to_fetch, true);
                        }
                    } else {
                         $value = $default_value;
                    }
                ...
                if ($field['type'] === 'section_title') {
                    echo '<div class="wpd-section-title-wrapper ' . esc_attr($width_class) . '" ' . $wrapper_attributes . '><h3>' . esc_html($field['label']) . '</h3></div>';
                    continue;
                }
                if ($field['type'] === 'html_content') {
                    echo '<div class="wpd-html-content-wrapper ' . esc_attr($width_class) . '" ' . $wrapper_attributes . '>' . wp_kses_post($field['options']) . '</div>';
                    continue;
                }
                ...
                <div class="<?php echo esc_attr($width_class); ?>" <?php echo $wrapper_attributes; ?>>
                    ...
                    switch ($field['type']) {
                        ...
                        case 'postal_code':
                        case 'national_id':
                            echo '<input type="' . esc_attr($field['type']) . '" id="' . esc_attr($field_id) . '" name="' . esc_attr($field_name) . '" value="' . esc_attr($value) . '"' . $input_attributes . '>';
                            break;
                        case 'textarea':
                            echo '<textarea id="' . esc_attr($field_id) . '" name="' . esc_attr($field_name) . '"' . $input_attributes . '>' . esc_textarea($value) . '</textarea>';
                            break;
                        case 'select':
                        ...
                        case 'radio':
                            ...
                            break;
                        case 'date':
                            echo '<input type="text" id="' . esc_attr($field_id) . '" name="' . esc_attr($field_name) . '" value="' . esc_attr($value) . '" class="wpd-date-picker"' . $input_attributes . '>';
                            break;
                        case 'time':
                            echo '<input type="time" id="' . esc_attr($field_id) . '" name="' . esc_attr($field_name) . '" value="' . esc_attr($value) . '"' . $input_attributes . '>';
                            break;
                        case 'image':
                        case 'file':
                            $button_text = ($field['type'] === 'image') ? __('انتخاب تصویر', 'wp-directory') : __('انتخاب فایل', 'wp-directory');
                            echo '<div class="wpd-file-field-wrapper">';
                            echo '<a href="#" class="wpd-button wpd-upload-file-button">' . esc_html($button_text) . '</a>';
                            ...
                            if (!empty($value)) {
                                $file_url = wp_get_attachment_url(absint($value));
                                if ($file_url) {
                                    echo '<p>' . __('فایل فعلی:', 'wp-directory') . ' <a href="' . esc_url($file_url) . '" target="_blank">' . esc_html(basename($file_url)) . '</a> <a href="#" class="wpd-remove-file">(' . __('حذف', 'wp-directory') . ')</a></p>';
                                }
                            }
                            echo '</div></div>';
                            break;
                        ...
                        case 'identity':
                            ...
                            break;
                        default:
                            echo '<input type="text" id="' . esc_attr($field_id) . '" name="' . esc_attr($field_name) . '" value="' . esc_attr($value) . '" class="regular-text"' . $input_attributes . '>';
                            break;
                    }
                    ?>
