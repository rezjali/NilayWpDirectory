...
class Directory_Admin {
    ...
    /**
     * BUG FIX: New helper function to generate consistent field IDs.
     * This ensures that the ID used in the input field matches the data-field-id on the button.
     */
    private function _get_field_id_attr($section, $id, $sub_section = null) {
        $full_id_name = "wpd_settings";
        $full_id_name .= $section ? "[{$section}]" : '';
        $full_id_name .= $sub_section ? "[{$sub_section}]" : '';
        $full_id_name .= $id ? "[{$id}]" : '';
    ...
    public function render_text_input_element($args){
        ...
        // BUG FIX: Use the helper function to generate a consistent ID
        $field_id_attr = $this->_get_field_id_attr($section, $id, $sub_section);
        $field_name = "wpd_settings[{$section}]" . ($sub_section ? "[{$sub_section}]" : "") . "[{$id}]";
