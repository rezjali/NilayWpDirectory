    ...
    if (!empty($fields) && is_array($fields)) :
        foreach ($fields as $field) :
            ...
            if (empty($meta_value) && $field['type'] !== 'html_content' && $field['type'] !== 'section_title') {
                continue; // Don't show fields without a value
            }
    ?>
    <div class="wpd-custom-fields-group">
        ...
        switch ($field['type']) {
            ...
            case 'gallery':
                ...
                if (!empty($image_ids)) :
                    ...
                    foreach ($image_ids as $id) :
                        ...
                        if ($image_url) :
                            echo '<a href="' . esc_url($image_url) . '" data-lightbox="wpd-gallery"><img src="' . esc_url($image_url) . '" alt="' . esc_attr($field['label']) . '"></a>';
                        endif;
                    endforeach;
                    echo '</div>';
                endif;
            ...
            case 'map':
                ...
                if (count($lat_lng) === 2) :
                    echo '<h4>' . esc_html($field['label']) . '</h4>';
                    echo '<div class="wpd-map-container" data-lat="' . esc_attr($lat_lng[0]) . '" data-lng="' . esc_attr($lat_lng[1]) . '"></div>';
                    // Inline script for Leaflet map initialization
                    ?>
                    <script>
                        document.addEventListener("DOMContentLoaded", function() {
                            var mapDiv = document.querySelector('.wpd-map-container');
                            if (mapDiv) {
